...
def is_criticality_balanced(temperature: float, neutrons_emitted:float) -> bool:
    ...
    temperature_times_neutrons: float = temperature * neutrons_emitted

    if temperature < 800 and neutrons_emitted > 500 and temperature_times_neutrons < 500000:  # noqa: SIM103
        return True
...
def fail_safe(temperature: float, neutrons_produced_per_second: float, threshold:float) -> str:
    ...
    temperature_times_neutrons:float = temperature * neutrons_produced_per_second

    if temperature_times_neutrons < threshold * 0.9:
        return "LOW"
    elif (threshold * -0.1) <= temperature_times_neutrons or temperature_times_neutrons <= (threshold * 0.1) or temperature_times_neutrons == threshold:
        return "NORMAL"
    else:
        return "DANGER"
