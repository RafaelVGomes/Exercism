...
Overview of exchanging currency when traveling: https://www.compareremit.com/money-transfer-tips/guide-to-exchanging-currency-for-overseas-travel/
"""
...
def exchangeable_value(budget: float, exchange_rate: float, spread:int, denomination:int) -> int:
    ...
    # Converts spread parameter integer to it's percentage value
    spread_percentage: float = spread / 100
    print("spread_percentage:", spread_percentage)

    # Determining the new exchange rate with spread
    exchange_rate_times_spread: float = exchange_rate * spread_percentage
    print("exchange_rate_times_spread:", exchange_rate_times_spread)
    exchange_rate_plus_spread: float = round(exchange_rate + exchange_rate_times_spread, 2)
    print(f"exchange_rate_plus_spread: {exchange_rate} + {exchange_rate_times_spread} = {exchange_rate_plus_spread}")

    # Calculates amount after exchange
    amount: float = exchange_money(budget, exchange_rate_plus_spread)
    print("amount:", amount)

    # Number of bills
    number_of_bills: int = get_number_of_bills(amount, denomination)

    # Calculates and returns the value of bills
    value_of_bills: int = get_value_of_bills(denomination, number_of_bills)
    print(f"value_of_bills: {value_of_bills}")
